...
// helper function with test store
...
// we need to connect Redux Store inside our setup for testing
...
// we need to connect to Redux Store inside our setup
// Passing 'store' as a prop to our Connect Component - input
const setup = (initialState = {}) => { // default initial state of our Test Redux Store
  ...
  // in our case, its the Input component wrapped with shallow wrapper below, now
  // to go one level inside of Input component to access html elements,
  // we need to dive one more time, add one more dive()

  // NOTE: Enzyme suggests that first connect components to Redux store &
